...
function formulaires_editer_auteur_sn_email2_charger($id_auteur,$retour=''){
...
function formulaires_editer_auteur_sn_email2_verifier($id_auteur,$retour=''){
	...
	if(count($erreurs) < 1){
		if (_request('passc')){
			if(!preg_match(sn_regex_txt_etendu(64,_PASS_LONGUEUR_MINI),_request('passc'))){
				$erreurs['passc'] = _T('sncore:regex_txt_etendu_nb',['nb'=>'64']).' '._T('sncore:info_longueur_motdepasse',['min'=>_PASS_LONGUEUR_MINI,'max'=>64]);
			}
		}
	}
	if(count($erreurs) < 1){
		if(_request('emailt') !== _request('emailc')){
			$erreurs['emailt'] = _T('sncore:erreur_comparaison_email');
		}
	}
	if(count($erreurs) < 1){
...
function formulaires_editer_auteur_sn_email2_traiter($id_auteur,$retour=''){
	...
	if ($controle === true && $autorisation === true) {
		$req = sql_fetsel(['id_auteur','sn_email2'],'spip_auteurs','id_auteur=' . intval($id_auteur));
		if($req){
			$mailex = $req['sn_email2'];
			$mailneo = _request('emailt');
			$p_envoi = [];
			$p_envoi['ref'] = 'notif_compte_modif';
			$p_envoi['objet'] = 'email2';
			$p_envoi['date_modif'] = date('Y-m-d H:i:d');
			if($mailex == ''){
				$p_envoi['objet'] = 'email2_ajout';
			}
			...
			$retours = [];
			if(sql_updateq('spip_auteurs',['sn_email2' => $mailneo],'id_auteur='.$id_auteur)){
